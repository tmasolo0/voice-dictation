...
### TASK-3.4: Fix переключения модели через вкладку "Модели"
- Статус: done (@Дмитрий, 2026-03-01)
- Файлы: ui/settings_dialog.py, app.py
- Детали:
  - Баг 1: Кнопка "Выбрать" синхронизирует _model_combo на вкладке "Распознавание"
  - Баг 2: Горячая перезагрузка модели через ModelManager.load_model() без перезапуска
  - recognition.model убран из RESTART_KEYS
